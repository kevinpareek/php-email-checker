 ...
 * @author Kevin Pareek
 * @package Okev
 * @copyright Copyright 2023 OKEV Project, Kevin Pareek.
 * @license https://opensource.org/licenses/MIT
 * @link https://kevinpareek.com/okev
 */

namespace KevinPareek\EmailChecker;

use KevinPareek\EmailChecker\Helpers\Helper;
...
class EmailChecker
{
    ...
    public $redirectDomainAccept = true;
    ...
    public function checkMxAndDnsRecord($email)
    {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            try {
                ...
                $result = 'valid';
                $details = $this->testMode == false ? 'MX record found but could not connect to server' : $e->getMessage();
            }
            return array($result, $details);
        } else {
            $result = 'invalid';
    ...
    public function checkDomain($email)
    {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($this->redirectDomainAccept != true && $httpcode === 301) {
                $domain = 'https://' . $this->splitEmail($email);
                $httpcode = $this->goCurl($domain);
            }
            $rcode = $this->redirectDomainAccept == true ? 400 : 300;
            if ($httpcode >= 200 && $httpcode < $rcode) {
                return true;
            } else {
                return false;
            }
